//#region node_modules/.nitro/vite/services/ssr/assets/catalog-BM8DsjeW.js
var LOCATIONS = [
	{
		id: "cairo",
		name: "Cairo Road, CBD",
...
var RESTAURANTS = [
	{
		...
		items: [
			{
				id: "nh1",
				name: "Nshima, chicken & relish",
				price: 65
			},
			{
				id: "nh2",
				name: "Ifisashi & village chicken",
				price: 78
			},
			{
				id: "nh3",
				name: "Kapenta platter",
				price: 55
			}
		]
	},
...
var DRIVERS = [
	{
		name: "Chanda Mwale",
		vehicle: "Toyota Corolla · BAC 4412"
	},
	{
		name: "Mutale Banda",
		vehicle: "Honda Fit · ALD 8821"
	},
	{
		name: "Thandi Phiri",
		vehicle: "Suzuki Swift · AAE 1904"
	},
	{
		name: "Joseph Zulu",
		vehicle: "Hyundai i20 · BAX 3308"
	},
	{
		name: "Grace Tembo",
		vehicle: "Kia Rio · ALC 7765"
	},
	{
		name: "Peter Mulenga",
		vehicle: "Toyota Hiace · BAC 2201"
	}
];
var DRIVER_JOBS = [
	{
		service: "rides",
		pickup: "Cairo Road, CBD",
		dropoff: "Kenneth Kaunda Airport",
...
function findLocation(name) {
	...
	return LOCATIONS.find((l) => l.name.toLowerCase() === n || l.name.toLowerCase().includes(n) || l.area.toLowerCase() === n);
}
function suggestLocations(query, exclude) {
	const q = query.trim().toLowerCase();
	const list = LOCATIONS.filter((l) => l.name !== exclude);
	if (!q) return list.slice(0, 6);
	return list.filter((l) => l.name.toLowerCase().includes(q) || l.area.toLowerCase().includes(q)).slice(0, 6);
}
function distanceOf(aName, bName) {
	const a = findLocation(aName);
	const b = findLocation(bName);
	if (!a || !b) return 12;
	return Math.max(4, Math.hypot(a.x - b.x, a.y - b.y));
}
function quoteFare(cls, pickup, dropoff) {
...
function pickDriver(seed = Date.now()) {
	return DRIVERS[seed % DRIVERS.length];
}
//#endregion
export { LOCATIONS as a, SERVICES as c, formatKw as d, pickDriver as f, DRIVER_JOBS as i, classesFor as l, suggestLocations as m, CORRIDORS as n, RESTAURANTS as o, quoteFare as p, DELIVERY_CLASSES as r, RIDE_CLASSES as s, CARGO_CLASSES as t, findLocation as u };
